    ...
    if not options.no_param:
        for i in range(num_frags):
            ...
            if not options.clobber and os.path.exists(param_file):
                ...
                write_param_file(os.path.join(options.dir, param_file), m, options.name, i+1, len(molfiles), options.max_confs, options.amino_acid)
    ...
    if not options.no_pdb:
        for i, molfile in enumerate(molfiles):
            ...
            if not options.clobber and os.path.exists(pdb_file):
                ...
                write_ligand_pdb(os.path.join(options.dir, pdb_file), m, molfile, options.name, options.center)
